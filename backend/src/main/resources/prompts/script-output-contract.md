...
- 尽量把内心独白转化为可表演的动作或对白。
- 必须保留所有章节中的主要冲突。
- 先根据“章节摘要”和“全局候选信息”统一人物、地点和冲突，再生成 scenes。
- 如果原文里出现多个称谓，characters 中统一成一个主名称，aliases 记录别名。
- scenes 必须覆盖主要剧情推进，不要把每章都机械拆成一个 scene。
- blocks 中 action 与 dialogue 交替组织，避免只有摘要没有可表演内容。
