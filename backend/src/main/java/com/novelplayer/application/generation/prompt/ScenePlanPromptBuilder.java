 ...
 */
@Component
public class ScenePlanPromptBuilder {

    private static final String SYSTEM_PROMPT_RESOURCE = "prompts/scene-plan/system.md";
    private static final String USER_PROMPT_RESOURCE = "prompts/scene-plan/user.md";
    ...
    /**
     * 默认构造器。
     * @throws IllegalStateException 当提示词文件不存在或加载失败时抛出。
     */
    public ScenePlanPromptBuilder() {
        this(
     ...
     * 构造器（包级私有，主要用于测试）。
     *
     * 允许通过构造参数注入自定义提示词，便于单元测试时
     * 使用固定或模拟的提示词内容。
     */
    ScenePlanPromptBuilder(String systemPrompt, String userPromptTemplate) {
        this.systemPrompt = normalizeTemplate(systemPrompt, "场景规划系统提示词");
        this.userPromptTemplate = normalizeTemplate(userPromptTemplate, "场景规划用户提示词");
    }
