 ...
 * 故事圣经阶段提示词构建器。
 *
 * 该构建器负责加载和管理故事圣经阶段所需的大语言模型提示词模板。
 * 加载后的提示词模板会进行规范化处理（去除首尾空白），并在构建时将输入的 JSON 数据格式化到用户提示词模板中。
 *
 * @see PromptMessages
 */
@Component
public class StoryBiblePromptBuilder {

    private static final String SYSTEM_PROMPT_RESOURCE = "prompts/story-bible/system.md";
    private static final String USER_PROMPT_RESOURCE = "prompts/story-bible/user.md";

    private final String systemPrompt;
    private final String userPromptTemplate;

    public StoryBiblePromptBuilder() {
        this(
                loadRequiredResource(SYSTEM_PROMPT_RESOURCE),
                loadRequiredResource(USER_PROMPT_RESOURCE)
        );
    }
    StoryBiblePromptBuilder(String systemPrompt, String userPromptTemplate) {
        this.systemPrompt = normalizeTemplate(systemPrompt, "故事圣经系统提示词");
        this.userPromptTemplate = normalizeTemplate(userPromptTemplate, "故事圣经用户提示词");
    }

    /**
     * 构建完整的提示词消息对象。
     *将输入的 JSON 数据格式化到用户提示词模板中，生成最终的系统提示词和用户提示词对。
     */
